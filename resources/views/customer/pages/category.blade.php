...
@section('head')
    <style>
        ...
        #filter-by-color .colors:hover{
            border: 2px solid #6BC637!important;
            padding: 0px;
            width: 38px;
            height: 38px;
            border-radius: 100%;
        ...
        <div class="container">
            ...
            <div class="row">
                ...
                <div id="aside" class="col-md-3">
                    <!-- aside widget -->
                    <div class="aside">
                        <h3 class="aside-title">{{ $category->category_name }}</h3>
                    ...
                    <!-- aside widget -->
                    <div class="aside">
                        <h3 class="aside-title">@lang('content.Filter by Price'):</h3>
                    ...
                    <!-- aside widget -->
                    <div class="aside">
                        <h3 class="aside-title">@lang('content.Filter by Brand'):</h3>
                        <ul class="list-links" id="filter-brand">
                            @foreach($brands as $brand)
                                @for($i=0; $i< count($brands_count); $i++)
                                    @if($brand->id === $brands_count[$i]['id'])
                                        <li><a class="filter_brand" href="javascript:void(0)" id="{{ $brand->id }}">{{ $brand->name }} ( {{ $brands_count[$i]['count'] }} )</a></li>
                                    @endif
                                @endfor
        ...
        $(function () {
            ...
            $(document).on('click', '.sort_btn', function () {
                ...
                $.ajax({
                    ...
                    method: 'GET',
                    data: {sorting_name: sorting_name, order: order, category_name: category_name},
            ...
            $(document).on('click', '.filter_size', function () {
                ...
                if(name.length > 0){
                    ...
                    $.ajax({
                        ...
                        method: 'GET',
                        data: {size: size, category_name: category_name},
                        success: function (data) {
                            console.log(data);
                            $('.products').hide().html(data).fadeIn('slow');
                        }
            ...
            $(document).on('click', '.filter_color', function () {
                ...
                $.ajax({
                    ...
                    method: 'GET',
                    data: {color: color, category_name: category_name},
                    success: function (data) {
                        console.log(data);
                        $('.products').hide().html(data).fadeIn('slow');
                    }
            ...
            if (slider) {
                ...
                document.getElementById('price-slider').addEventListener('click', function () {
                    ...
                    $.ajax({
                        ...
                        type: "GET",
                        data: {values: values, category_name: category_name},
                        success: function (data) {
                            $('.products').hide().html(data).fadeIn('slow');
                        }
                    });
